    ...
    match stream.read(&mut buffer) {
        Ok(size) => {
            ...
            let (status_line, content) = match &*request {
                r if r.start_with("POST /users") => handle_post_request(r),
                r if r.start_with("GET /users/") => handle_get_request(r),
                r if r.start_with("GET /users") => handle_get_all_request(r),
                r if r.start_with("PUT /users/") => handle_put_request(r),
                r if r.start_with("DELETE /users/") => handle_delete_request(r),
